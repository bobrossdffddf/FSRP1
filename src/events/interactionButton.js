    ...
    async execute(interaction, client) {
        ...
        if (interaction.isButton()) {
                ...
                if (freshTicket?.ticketMessageId) {
                    ...
                    if (msg) {
                        try {
                            ...
                            await msg.edit({ components: [container], files, flags: CV2_FLAG });
                        } catch (e) {
                            console.warn('[Claim] edit failed:', e.message);
                        }
                    }
                }
                return;
                ...
                if (freshTicket?.ticketMessageId) {
                    ...
                    if (msg) {
                        try {
                            ...
                            console.warn('[Unclaim] edit failed:', e.message);
                        }
                    }
                }
                return;
            ...
            if (interaction.customId.startsWith('ticket_close_force:')) {
                ...
                // Resolve the ephemeral spinner BEFORE channel deletion so it doesn't hang.
                await interaction.editReply({ content: 'Closing ticket — transcript being generated.' }).catch(() => {});
                const result = await closeTicket(interaction.channel, ticket, interaction.user, client);
                if (!result.ok) {
                    await interaction.followUp({
                        content: `Ticket close failed: \`${result.error}\`.`,
                        flags: 64,
                    }).catch(() => {});
                }
                return;
            ...
            if (interaction.customId.startsWith('ticket_close_accept:')) {
                ...
                // Resolve ephemeral spinner BEFORE channel deletion.
                await interaction.editReply({ content: 'Close request accepted — closing ticket.' }).catch(() => {});
                const acceptResult = await closeTicket(interaction.channel, ticket, interaction.user, client);
                if (!acceptResult.ok) {
                    await interaction.followUp({
                        content: `Close failed: \`${acceptResult.error}\`.`,
                        flags: 64,
                    }).catch(() => {});
                }
                return;
