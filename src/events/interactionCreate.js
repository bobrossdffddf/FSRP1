...
const SELF_PERMISSIONED_COMMANDS = ['setup', 'ticket-setup', 'infraction', 'promote', 'staffrequest', 'statlookup', 'my-infractions', 'escalate', 'rename', 'close', 'ticket'];
        ...
        try {
            ...
            if (interaction.isModalSubmit()) {
                ...
                if (interaction.customId === 'staff_report_modal') {
                    ...
                    try {
                        const selectedUsers = interaction.fields.getSelectedUsers('reported_user');
                        if (selectedUsers && selectedUsers.size > 0) {
                            reportedUserId = selectedUsers.first().id;
                        }
                    } catch (e) {
                        console.warn('[StaffReport] getSelectedUsers error:', e.message);
                    }
                    ...
                    try {
                        evidenceFiles = interaction.fields.getUploadedFiles('evidence_files');
