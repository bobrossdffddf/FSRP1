...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        // Send priority button and ticket panel in parallel after settings are saved
        client.settings.set(guildId, { ...existing, ...updates });
        
        if (priorityChannel || ticketPanelChannel) {
            await Promise.all([
                priorityChannel ? priorityChannel.send({ embeds: [buildPriorityEmbed(false)], components: [buildPriorityRow(false)] }).then(sent => { updates.priorityMessageId = sent.id; }).catch(e => {
                    console.error('[Setup] Failed to send priority button:', e.message);
                }) : Promise.resolve(),
                ticketPanelChannel ? ticketPanelChannel.send({ components: [buildTicketPanelContainer()], files: buildTicketPanelFiles(), flags: TICKET_FLAGS }).then(sent => { updates.ticketPanelMessageId = sent.id; }).catch(e => {
